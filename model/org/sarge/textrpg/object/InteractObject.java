...
public class InteractObject extends WorldObject {
	...
	public static class Descriptor extends ObjectDescriptor {
		...
		public Descriptor(ObjectDescriptor descriptor, Set<Interaction> interactions, int str, boolean removes) {
			...
			if(interactions.contains(Interaction.EXAMINE)) throw new IllegalArgumentException("EXAMINE invalid for an interact object");
			verifyResetable();
	...
	protected void interact(Interaction action, Parent parent) throws ActionException {
		...
		final Descriptor descriptor = (Descriptor) super.descriptor;
		if(!descriptor.interactions.contains(action)) throw new ActionException("interact.invalid.action");
		if(model.isOpen()) throw new ActionException("interact.done." + action);
		...
		final Event reset = () -> {
			model.toggle();
			if(descriptor.removes) setParentAncestor(parent);
		};
		Control.QUEUE.add(reset, descriptor.getProperties().getResetPeriod());
	}
}
