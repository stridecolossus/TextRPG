...
public class Equipment {
	...
	public void hold(WorldObject obj) throws ActionException {
		...
		if(equipment.get(DeploymentSlot.MAIN_HAND) == null) {
			final boolean main = obj.getDescriptor().getEquipment().getDeploymentSlot().filter(slot -> slot == DeploymentSlot.MAIN_HAND).isPresent();
			if(main) {
	...
	protected boolean equip(WorldObject obj) throws ActionException {
		final ObjectDescriptor.Equipment desc = obj.getDescriptor().getEquipment();
		final DeploymentSlot slot = desc.getDeploymentSlot().orElseThrow(() -> new ActionException("equipment.cannot.equip"));
		if(slot.isContainerSlot()) {
			...
			case OFF_HAND:
				...
				if(desc.isTwoHanded()) {
					...
					if(otherObject != null) {
						if(otherObject.getDescriptor().getEquipment().isTwoHanded()) throw new ActionException("equipment.two.handed");
					}
				}
				break;
			}
	...
	protected void remove(WorldObject obj) throws ActionException {
		final ObjectDescriptor.Equipment desc = obj.getDescriptor().getEquipment();
		final DeploymentSlot slot = desc.getDeploymentSlot().orElseThrow(() -> new ActionException("equipment.not.equipped"));
		if(slot.isContainerSlot()) {
