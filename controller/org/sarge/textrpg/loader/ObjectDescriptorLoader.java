...
public class ObjectDescriptorLoader {
	...
	public ObjectDescriptor load(TextNode node) {
		...
		if(slot == null) {
			node.optionalChild("equipment").ifPresent(e -> loadEquipment(e, builder));
		}
		else {
			builder.slot(SLOT.convert(slot));
		}
	 ...
	 */
	private void loadEquipment(TextNode node, Builder builder) {
		builder
